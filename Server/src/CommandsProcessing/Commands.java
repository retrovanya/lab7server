...
static Flat f;
    ...
    public static String add(Flat flatat) throws SQLException {
        ...
        try  {
            ...
            preparedStatement.setString(10, flatat.getHouse().getName());
    ...
    public static String updateId(long x, Flat flat) throws SQLException {
        locker.lock();
        ...
        try {
            ...
            while (resultSet.next()) {
                if ((resultSet.getInt("id")) == x) {
                    ...
                    resultSet.updateString("house_name", flat.getHouse().getName());
    ...
    public static String removeByHouseAge (long uu){
        ...
        Connection connection = null;
        String answer ="";
        String select = "DELETE FROM PersonLists WHERE house_age = " + uu + " AND login = " + "'" + Receiver.login + "'" + ";";
        try  {
            connection = DriverManager.getConnection(Receiver.URL, Receiver.USERNAME, Receiver.PASSWORD);
            preparedStatement = connection.prepareStatement(select);
            int j = preparedStatement.executeUpdate();
            if (j==1){
                answer = "Элементы, возраст дома которых равен " + uu+ " созданы пользователем с логином "+ Receiver.login +", успешно удалены.";
            } else if (j == 0){
                answer = "Элементы, возраст дома которых равен " + uu + " не найдены.";
            }else{
                answer = "Элементы,  которых превышает " + uu + " созданы пользователем с логином "+ Receiver.login +", успешно удалены.";
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public static String removeGreater (long uu){
        ...
        Connection connection = null;
        String answer ="";
        String select = "DELETE FROM PersonLists WHERE id > " + uu + " AND login = " + "'" + Receiver.login + "'" + ";";
        try  {
            connection = DriverManager.getConnection(Receiver.URL, Receiver.USERNAME, Receiver.PASSWORD);
            preparedStatement = connection.prepareStatement(select);
            int j = preparedStatement.executeUpdate();
